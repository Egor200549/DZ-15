...
const Card: FC<Props> = ({ data, fetchData, setFavourites, type }) => {
    ...
    const deleteFavourite = async (id: number) => {
        try {
            const response = await fetch('https://a4a09c5ecb7ce7ac.mokky.dev/favourities/' + id,
                {
                    method: "DELETE",
                    headers: {
                        "Content-Type": 'application/json',
    ...
    return (
        <div className="card">
            ...
            <div className="card__buttons">
                {
                    type === "catalog" ?
                        <button className="card__button delete" onClick={() => deleteItem(data.id)} title="Удалить товар из каталога">
                ...
                {
                    type === "catalog" ?
                        <button className="card__button edit" onClick={edit} title="Редактировать">
                ...
                {
                    type === "catalog" ?
                        <button className="card__button yellow" onClick={addFavourite} title="Добавить в избранное">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M21.5,9.757l-5.278,4.354L17.871,21.5,12,17.278,6.129,21.5l1.649-7.389L2.5,9.757l6.333-.924L12,2.5l3.167,6.333Z" fill="#fff" />
                            </svg>
                        </button> : null
                }
                {
                    type === 'client' ?
                        <button className="card__button yellow" onClick={() => deleteFavourite(data.id)} title="Удалить из избранного">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M21.5,9.757l-5.278,4.354L17.871,21.5,12,17.278,6.129,21.5l1.649-7.389L2.5,9.757l6.333-.924L12,2.5l3.167,6.333Z" fill="red" />
                            </svg>
                        </button> : null
                }
            </div>
